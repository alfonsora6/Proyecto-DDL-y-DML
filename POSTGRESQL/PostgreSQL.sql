...
CREATE USER alfonso WITH ENCRYPTED PASSWORD 'alfonso';
GRANT ALL PRIVILEGES ON DATABASE liga TO alfonso;
\c liga
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO alfonso;
...
--ALGUNAS OPCIONES DE PSQL

\l → Mostrar base de datos existentes.
\q → Salir de cualquier sitio.
\d → Mostrar las tablas de mi BD.
\d nombreTabla → Mostrar los campos de una tabla.
...
(
    ...
    CONSTRAINT ck_iniciales CHECK(Nombre ~ '^[A-Z]'),
    CONSTRAINT ck_edad CHECK((2022 - EXTRACT(YEAR FROM FechaNac)) <= 36),
...
(
    ...
    FechaNac DATE,
    NombreEquipo VARCHAR(20),
    CONSTRAINT ck_dni CHECK(DNI ~ '^[0-9]{8}[A-Za-z]$'),
    CONSTRAINT ck_inicialesmayus CHECK(Nombre ~ '^[A-Z]'),
...
ALTER COLUMN Clima TYPE VARCHAR(20),
ALTER COLUMN Clima SET Default 'Despejado';
...
ALTER TABLE Equipos ADD CONSTRAINT ck_ciudad CHECK(Ciudad ~ '^[A-Z]');
...
--No es necesario ejecutar un 'COMMIT;' puesto que PostgreSQL tiene de forma predeterminada el valor de confirmación en ON para guardar los cambios de cada transacción. 
